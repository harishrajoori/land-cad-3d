...
export function planSVG(m: HouseConcept, floor: number) {
  ...
  svg += `<text class="label" x="${p.x + p.w / 2}" y="${p.z + p.d / 2 - .5}">Outdoor Patnam</text><text class="small" x="${p.x + p.w / 2}" y="${p.z + p.d / 2 + 1}">${p.w.toFixed(0)} × ${p.d.toFixed(0)} ft assumed</text>`;
  // Parking markers reflect the model's (possibly degraded) counts.
  for (const f of m.furniture.filter(f => /^(car_|motorcycle)/.test(f.catalog) && f.floor === 0)) {
    const car = f.catalog.startsWith('car_');
    svg += rect(f.x - (car ? 3 : 1.3), f.z - (car ? 8 : 3.4), car ? 6 : 2.6, car ? 16 : 6.9, car ? '#e8e1d4' : '#d2c5ad', '#788579');
  }
